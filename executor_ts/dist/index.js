...
import { getTipAccounts, sendBundle } from "./jito.js";
import { buildBuyTx, buildSellTx, buildTipTx, fetchBondingCurveState, quoteBuy, quoteSellAll, quoteSellTokenAmount, } from "./pumpfun.js";
dotenv.config();
...
function parseControlTradingDryRun() {
...
function parseSecretsHeliusKey() {
    ...
    for (const raw of txt.split(/\r?\n/)) {
        ...
        if (inHelius && indent <= heliusIndent)
            inHelius = false;
        if (!inHelius)
            continue;
        ...
        if (!v)
            continue;
        if (k === "api_key_executor")
            executor = v;
        else if (k === "api_key_python")
            python = v;
        else if (k === "api_key")
            fallback = v;
    }
...
function resolveRpcUrl() {
    ...
    const isHelius = (u) => /helius-rpc\.com/i.test(u);
    if (envUrl && envUrl.trim()) {
        const u = envUrl.trim();
        if (!isHelius(u) || /api-key=/i.test(u) || !key)
            return u;
        try {
            const url = new URL(u);
...
const LIVE_ENABLED = process.env.EXECUTOR_LIVE !== undefined
 ...
 */
function callMaybeOpts(fn, positional, opts) {
...
app.get("/state", async (req, res) => {
    try {
        ...
        if (!mintStr)
            return res.status(400).json({ ok: false, message: "missing mint" });
...
app.get("/quote", async (req, res) => {
    try {
        ...
        const slipBps = Number(req.query.slippage_bps ??
        ...
        if (side === "buy") {
            ...
            const q = await callMaybeOpts(quoteBuy, [connection, payer.publicKey, mint, solLamports, slippageBps], { connection, payer: payer.publicKey, mint, solInLamports: solLamports, slippageBps });
            ...
            if (tokenInStrTrim && tokenInStrTrim !== "0") {
                ...
                const q = await callMaybeOpts(quoteSellTokenAmount, [connection, payer.publicKey, mint, tokenIn, slippageBps], { connection, payer: payer.publicKey, mint, tokenIn, slippageBps });
                return res.json({ ok: true, quote: q });
            }
            // Otherwise quote SELL ALL based on wallet balance.
            const q = await callMaybeOpts(quoteSellAll, [connection, payer.publicKey, mint, slippageBps], { connection, payer: payer.publicKey, mint, slippageBps });
            return res.json({ ok: true, quote: q });
        }
    }
    catch (e) {
...
app.post("/trade", async (req, res) => {
    try {
        ...
        const amountInSol = Number(o.amount_in ?? o.solIn ?? 0);
        const slippageBps = Number(o.slippage_bps ?? Math.round(Number(o.slippage ?? 1) * 100));
        const useJito = Boolean(o.useJito ?? true);
        const simulate = Boolean(o.simulate ?? false);
        const bodyDryRun = o.dry_run;
        const reqDryRun = typeof bodyDryRun === "boolean" ? bodyDryRun : o.dryRun;
        const dryRun = typeof reqDryRun === "boolean" ? reqDryRun : DEFAULT_DRY_RUN;
        ...
        let tx;
        let quoteMeta = undefined;
        if (side === "buy") {
            tx = await callMaybeOpts(buildBuyTx, [connection, payer, mint, solLamports, slippageBps], { connection, payer, mint, amountInLamports: solLamports, slippageBps });
            quoteMeta = tx.__quote;
        }
        else if (side === "sell") {
            tx = await callMaybeOpts(buildSellTx, [connection, payer, mint, slippageBps], { connection, payer, mint, slippageBps });
        }
        else {
            return res
                .status(400)
        ...
        if (JITO_ENABLED && useJito) {
            ...
            const tipTx = await callMaybeOpts(buildTipTx, [connection, payer, tips, JITO_TIP_LAMPORTS], { connection, payer, tipAccounts: tips, tipLamports: JITO_TIP_LAMPORTS });
        ...
        const msg = e instanceof Error ? e.message : String(e);
        return res.status(500).json({ ok: false, dry_run: true, message: msg });
    }
});
