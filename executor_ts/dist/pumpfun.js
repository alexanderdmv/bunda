...
function asBN(v) {
    ...
    if (typeof v === "number")
        return new BN(Math.trunc(v).toString());
...
function pickPdaPubkey(r) {
    if (!r)
        return null;
    if (r instanceof PublicKey)
        return r;
    if (Array.isArray(r) && r[0] instanceof PublicKey)
        return r[0];
    if (r.publicKey instanceof PublicKey)
        return r.publicKey;
    if (r.pda instanceof PublicKey)
        return r.pda;
    if (typeof r === "string") {
        try {
...
function deriveBondingCurvePda(base, mint, tokenProgram) {
...
async function getAccountInfoSafe(connection, pubkey) {
    if (!pubkey)
        return null;
    try {
...
const FEECFG_CACHE = new Map();
const STATE_CACHE = new Map();
const BC_CACHE = new Map();
function getCached(m, key, ttl) {
    const e = m.get(key);
    if (!e)
        return undefined;
...
function lamportsStrToSol(lamportsStr) {
    try {
...
function getSdk(connection) {
    const key = connection.rpcEndpoint ?? "rpc";
    ...
    if (typeof OnlinePumpSdk !== "function") {
        throw new Error(`Pump SDK init failed: OnlinePumpSdk missing. exportsHint=${exportsHint()}`);
    }
    // Offline SDK instance: prefer exported singleton (PUMP_SDK), else construct PumpSdk.
    const offline = (PUMP_SDK && typeof PUMP_SDK === "object") ? PUMP_SDK
        : (typeof PumpSdk === "function" ? new PumpSdk() : null);
    if (!offline) {
        throw new Error(`Pump SDK init failed: no offline SDK. exportsHint=${exportsHint()}`);
    }
    ...
    const combo = {
        ...
        fetchBuyState: async (mint, user, tokenProgram) => {
        ...
        fetchSellState: async (mint, user, tokenProgram) => {
...
async function getTokenBalanceBN(connection, mint, owner, tokenProgram) {
...
export async function quoteBuy(connection, payerPublicKey, mint, solInLamports, slippageBps) {
    const base = getPumpBase();
    const fn = base.getBuyTokenAmountFromSolAmount;
    if (typeof fn !== "function") {
        throw new Error(`pump-sdk missing getBuyTokenAmountFromSolAmount. exportsHint=${exportsHint()}`);
    }
 ...
 */
export async function fetchBondingCurveState(connection, mint) {
    const key = connection.rpcEndpoint ?? "rpc";
    ...
    const bc = await sdk.online.fetchBondingCurve(mint);
...
export async function quoteSellAll(connection, payerPublicKey, mint, slippageBps) {
    const base = getPumpBase();
    const fn = base.getSellSolAmountFromTokenAmount;
    if (typeof fn !== "function") {
        throw new Error(`pump-sdk missing getSellSolAmountFromTokenAmount. exportsHint=${exportsHint()}`);
    }
...
export async function quoteSellTokenAmount(connection, payerPublicKey, mint, tokenIn, slippageBps) {
    const base = getPumpBase();
    const fn = base.getSellSolAmountFromTokenAmount;
    if (typeof fn !== "function") {
        throw new Error(`pump-sdk missing getSellSolAmountFromTokenAmount. exportsHint=${exportsHint()}`);
    ...
    const bondingCurve = st?.bondingCurve ?? null;
    if (!bondingCurve)
        throw new Error("bondingCurve missing (cannot quote sell)");
    const mintSupply = bondingCurve?.tokenTotalSupply ?? global?.tokenTotalSupply;
...
export async function buildBuyTx(connection, payer, mint, solInLamports, slippageBps) {
    const base = getPumpBase();
    ...
    if (typeof offline.buyInstructions !== "function") {
        throw new Error(`offline SDK missing buyInstructions(). keys=${Object.keys(offline).join(",")}`);
    }
    ...
    if (st?.bondingCurve)
        args.bondingCurve = st.bondingCurve;
    if (bondingCurveAccountInfo)
        args.bondingCurveAccountInfo = bondingCurveAccountInfo;
    if (associatedUserAccountInfo)
        args.associatedUserAccountInfo = associatedUserAccountInfo;
    // Some SDK versions require fee config during instruction build.
    if (feeConfig) {
        if (feeConfig.feeConfig)
            args.feeConfig = feeConfig.feeConfig;
        else
            args.feeConfig = feeConfig;
        if (feeConfig.feeConfigAccountInfo)
            args.feeConfigAccountInfo = feeConfig.feeConfigAccountInfo;
        if (feeConfig.accountInfo && !args.feeConfigAccountInfo)
            args.feeConfigAccountInfo = feeConfig.accountInfo;
    }
...
export async function buildSellTx(connection, payer, mint, slippageBps) {
    const base = getPumpBase();
    ...
    if (typeof offline.sellInstructions !== "function") {
        throw new Error(`offline SDK missing sellInstructions(). keys=${Object.keys(offline).join(",")}`);
    }
    const args = {
        global,
        mint,
...
export async function buildTipTx(connection, payer, tipAccounts, tipLamports) {
    const { SystemProgram } = await import("@solana/web3.js");
    if (!tipAccounts?.length)
        throw new Error("No tip accounts");
